...
rutaProduct.get('/', async (req, res) => {
 ...
 if (req.query.limit) {
  prods = prods.slice(0, parseInt(req.query.limit))
}
res.send(prods)
})
...
rutaProduct.post('/', async (req, res) => {
  try {
    ...
    res.status(200).send({ respuesta: 'OK', mensaje: JSON.stringify(respPost) + ' Ingresado a FS y '  + JSON.stringify(prod)  + 'Ingresado a DB' })
} catch (error) {
